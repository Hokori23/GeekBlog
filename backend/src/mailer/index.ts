 ...
 * @description 广播邮件
 */
const broadcastMail = async (subject: string, html: string, accepters: MailAccepter[]) => {
...
const broadcastMails = async (attributes: BroadcastMailsAttribute[]) => {
  const promiseArr: Array<Promise<any>> = attributes.map(async ({ subject, html, accepter }) => {
    // 添加to, subject, HTML文本内容属性
    return await send(subject, html, accepter)
  })
  return await Promise.all(promiseArr)
...
const send = async (
  ...
  if (count > 0 && isDev)
    ...
    console.log(
      chalk.yellow(`${accepter.name} <${accepter.address}>: 第 ${count} 次尝试重发邮件...`),
    )
  ...
  try {
    ...
    if (count >= MAILER_OPTIONS.maxTryCount) {
      isDev &&
        // eslint-disable-next-line no-console
        console.log(chalk.red(`邮件发送失败，时间：${moment().format('YYYY-MM-DD hh:mm:ss')}`))
      throw new Error(String(e)) // 调用函数者需处理error
    } else {
      await send(subject, html, accepter, count, e as any)
    }
  }
}
