...
export const OutputTemplate = async (newPostInfo: NewPostInfoAttribute): Promise<string> => {
  const { title, postTitle, newPostUrl, userName, blogConfig } = newPostInfo
  if (isDev || !obj.cssOutputString) {
    obj.cssOutputString = (await fsp.readFile(path.resolve(__dirname, 'template.css'))).toString()
  }
  if (isDev || !obj.ejsOutputString) {
    obj.ejsOutputString = (await fsp.readFile(path.resolve(__dirname, 'template.ejs'))).toString()
  }
