...
import { ROUTER_WHITE_LIST, ROUTER_ADMIN_ALLOW, ROUTER_SUPER_ADMIN_ALLOW } from '@utils/const'
export default (req, res, next) => {
  ...
  if (group === Group.SUBSCRIBER) {
    if (ROUTER_SUPER_ADMIN_ALLOW.includes(path) || ROUTER_ADMIN_ALLOW.includes(path)) {
