...
const Activate = async (user: User): Promise<Restful> => {
  ...
  try {
    ...
    if (values.filter((v) => isDef(v)).length) {
      return new Restful(CodeDictionary.SERVICE_ERROR__CAPTCHA_USER_EXISTED, '账号已存在')
    }

    // 生成验证码
    const captcha = md5Crypto(`${userAccount} - ${email} - ${Date.now().toString()}`).slice(0, 8)
    ...
    const MailConfirmInfo: MailConfirmAttributes = {
      title: `确认激活邮箱 -- ${(formattedOption.system?.blogName as string) || '某博客'}`,
    ...
    // 发送邮件
    await send(MailConfirmInfo.title, await template.MailConfirm(MailConfirmInfo), {
      name: userName,
      address: email,
    })
    await t.commit()
    ...
    await t.rollback()
    return new Restful(CodeDictionary.COMMON_ERROR, `发送激活邮件失败, ${String(e.message)}`)
  }
}
export default {
  Activate,
