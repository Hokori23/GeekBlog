...
const Save = async (options: Option[]): Promise<Restful> => {
  ...
  try {
    const _options = await Action.CreateBulk__Update(options, t)
    await t.commit()
    return new Restful(CodeDictionary.SUCCESS, '保存设置成功', _options)
  } catch (e: any) {
    await t.rollback()
    return new Restful(CodeDictionary.COMMON_ERROR, `保存设置失败, ${String(e.message)}`)
  }
}
