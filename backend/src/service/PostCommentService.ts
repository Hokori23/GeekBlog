...
const Create = async (comment: PostCommentWithAuthor): Promise<Restful> => {
  ...
  try {
    ...
    if (existedPost.isLocked) {
      return new Restful(CodeDictionary.SERVICE_ERROR__COMMENT_POST_IS_LOCKED, '该贴评论区已封锁')
    }
    ...
    if (isRegistered) {
      ...
      return new Restful(CodeDictionary.SERVICE_ERROR__COMMENT_EMAIL_NEEDED, '评论需要email')
    } else {
      comment.uid = 0
    }

    // 添加帖子评论
    const _comment = await Action.Create(comment, t)

    // 判断是否有父评论
    if (isDef(_comment.parentId) && isDef(_comment.rootId)) {
      const parentComment = existedPost.postComments?.find((v) => v.id === _comment.parentId)
      const rootComment = existedPost.postComments?.find((v) => v.id === _comment.rootId)
      ...
      //   (v) => v.parentId === _comment.parentId && v.email !== _comment.email,
      // )
      ...
      const replyComments =
        parentComment.email === _comment.email // 判断回复的评论是不是自己的
          ? []
          : [parentComment]
      ...
      if (replyComments?.length) {
        const attributes: BroadcastMailsAttribute[] = await Promise.all(
          replyComments.map(async (v) => {
            // 接受邮件人的userName
            const userName =
              v.uid === 0 ? v.email : ((await UserAction.Retrieve('id', v.uid)) as User).userName
            return {
              subject: title,
...
const Delete = async (id: number): Promise<Restful> => {
  try {
    ...
    return new Restful(CodeDictionary.COMMON_ERROR, `删除评论失败, ${String(e.message)}`)
  }
}
...
const Like = async (id: number): Promise<Restful> => {
  try {
    ...
    return new Restful(CodeDictionary.COMMON_ERROR, `点赞评论失败, ${String(e.message)}`)
  }
}
...
const Dislike = async (id: number): Promise<Restful> => {
  try {
    ...
    return new Restful(CodeDictionary.COMMON_ERROR, `踩评论失败, ${String(e.message)}`)
  }
}
