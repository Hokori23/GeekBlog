...
const Create = async (tag: PostTag): Promise<Restful> => {
  try {
    ...
    if (isDef(values[0])) {
      return new Restful(CodeDictionary.SERVICE_ERROR__TAG_EXISTED, '已存在同名标签')
    }
    if (isDef(values[1])) {
      return new Restful(CodeDictionary.SERVICE_ERROR__TAG_EXISTED, '已存在相同slug的标签')
    }
    const _tag = await Action.Create(tag)
    return new Restful(CodeDictionary.SUCCESS, '创建标签成功', _tag.toJSON())
  } catch (e: any) {
    return new Restful(CodeDictionary.COMMON_ERROR, `创建标签失败, ${String(e.message)}`)
  }
}
...
const RetrieveBySlug = async (slug: string): Promise<Restful> => {
  try {
    ...
    if (isUndef(tag)) {
      return new Restful(CodeDictionary.RETRIEVE_ERROR__POST_NON_EXISTED, '标签不存在')
    }
...
const Edit = async (tag: PostTag): Promise<Restful> => {
  try {
    ...
    if (isUndef(existedTag)) {
      return new Restful(CodeDictionary.SERVICE_ERROR__TAG_NON_EXISTED, '此标签已不存在')
    }
    const _tag = await Action.Update(existedTag, tag)
    return new Restful(CodeDictionary.SUCCESS, '编辑标签成功', _tag.toJSON())
  } catch (e: any) {
    return new Restful(CodeDictionary.COMMON_ERROR, `编辑标签失败, ${String(e.message)}`)
  }
}
...
const Delete = async (id: number): Promise<Restful> => {
  try {
    ...
    return new Restful(CodeDictionary.COMMON_ERROR, `删除标签失败, ${String(e.message)}`)
  }
}
