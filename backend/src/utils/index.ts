 ...
 * @returns { boolean }
 */
export const isUndef = (v: unknown): v is undefined | null => v === undefined || v === null
 ...
 */
export const debounce = (callback: (...args: any[]) => any, delay: number): Function => {
  return (() => {
...
export const md5Crypto = (v: string): string => {
  ...
  if (isDef(onceCryptLength) && onceCryptLength > 0) {
    while (str) {
      const tempV = str.slice(0, onceCryptLength)
      str = str.slice(onceCryptLength)
      md5.update(`${tempV} - `)
    }
  ...
  if (isDef(cryptCount) && cryptCount > 0) {
    if (vLength <= cryptCount) {
      return md5.update(str).digest(digest)
    } else {
...
export const findSrcFiles = (filePath: string, filesList: File[], reg: RegExp) => {
...
export class Restful {
  ...
  public data?: any
  public constructor(code: number, message: string, data: any = null) {
    this.code = code
